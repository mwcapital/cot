...
def create_participant_behavior_clusters(df, instrument_name):
    ...
    tab1, tab2, tab3 = st.tabs(["Behavioral Clustering", "Market State Clustering", "Trend-Aware Clustering"])
...
def create_behavioral_clustering(df, instrument_name):
    ...
    with st.expander("📖 Understanding Participant Behavior Clusters", expanded=False):
        ...
        - **What**: Category label encoded as a number (NOT a measured behavior)
        - **Values**: Long categories = +1, Short categories = -1
        - **Important**: CFTC reports all positions as positive numbers. This feature simply
          encodes which side (long/short) the data point belongs to.
        - **Effect on clustering**: Forces long categories to cluster separately from short categories
        - **In Clusters**: Average near 0 = mixed longs/shorts, near +1 = mostly longs, near -1 = mostly shorts
        - **Trade-off**: Including this prevents discovering when opposite sides show similar behavior.
          Use "Exclude Direction" checkbox to find cross-side behavioral similarities.
        ...
        ### 4. **Concentration (CFTC 4-Trader Concentration)**
        - **What**: % of open interest held by the top 4 largest traders
        - **Source**: Direct from CFTC API (`conc_gross_le_4_tdr_long/short`)
        - **Example**: 45% means top 4 traders control 45% of all positions
        - **Interpretation**:
          - 60-100%: Highly concentrated (few large players dominate)
          - 40-60%: Moderately concentrated
          - 20-40%: Normal distribution
          - 0-20%: Highly fragmented (many small traders)
        - **Why this matters**: Direct measure of market concentration, not a proxy
...
2. Direction = +1 for Long categories, -1 for Short categories (category label, not measured)
3. Activity = |change_in_positions| / Open_Interest × 100
4. Concentration = conc_gross_le_4_tdr_long/short from CFTC API (% held by top 4 traders)
...
- Note: Direction feature forces longs/shorts apart; disable to find cross-side patterns
        """, language='python')
        ...
        for window_result in window_results:
            ...
            for idx, row in df_window_temp.iterrows():
                ...
                for cat_name, pos_col, trader_col in categories:
                    ...
                    # Feature 4: Concentration (using CFTC 4-trader concentration data)
                    # This is the actual % of open interest held by the top 4 traders
                    # Much more meaningful than inverted trader count percentile
                    conc_map = {
                        'Commercial Long': 'conc_gross_le_4_tdr_long',
                        'Commercial Short': 'conc_gross_le_4_tdr_short',
                        'Non-Commercial Long': 'conc_gross_le_4_tdr_long',
                        'Non-Commercial Short': 'conc_gross_le_4_tdr_short'
                    }
                    conc_col = conc_map.get(cat_name)
                    if conc_col and conc_col in row.index and not pd.isna(row[conc_col]):
                        concentration = float(row[conc_col])
                    else:
                        # Fallback: use trader count percentile if CFTC concentration not available
                        hist_data = df_cluster[
                            (df_cluster['report_date_as_yyyy_mm_dd'] >= pd.Timestamp('2010-01-01')) &
                            (df_cluster['report_date_as_yyyy_mm_dd'] <= date)
                        ]
            ...
            if len(window_cluster_data) > 0:
                ...
                all_window_clusters.append({
                    ...
                    'scaler': scaler  # Store scaler for proper cross-window comparison
                })
    ...
    for idx, row in df_window.iterrows():
        ...
        for cat_name, pos_col, trader_col in categories:
            ...
            if conc_col and conc_col in row.index and not pd.isna(row[conc_col]):
                ...
                # Fallback: use trader count percentile if CFTC concentration not available
                # Still uses look-ahead-safe calculation
                hist_data = df_cluster[
                    (df_cluster['report_date_as_yyyy_mm_dd'] >= pd.Timestamp('2010-01-01')) &
                    (df_cluster['report_date_as_yyyy_mm_dd'] <= date)
                ]
    ...
    if window_selection == "compare_all" and 'comparison_data' in locals() and comparison_data['all_window_clusters']:
        ...
        for i in range(len(comparison_data['all_window_clusters']) - 1):
            ...
            # IMPORTANT: Inverse transform to RAW feature space before comparing
            # Each window has its own scaler, so scaled centers are NOT comparable
            curr_centers_scaled = curr_window['centers']
            next_centers_scaled = next_window['centers']

            # Inverse transform to raw feature space
            curr_centers_raw = curr_window['scaler'].inverse_transform(curr_centers_scaled)
            next_centers_raw = next_window['scaler'].inverse_transform(next_centers_scaled)

            # Calculate average distance between corresponding clusters (in RAW space)
            # Normalize by feature scales to make distance interpretable
            feature_ranges = np.ptp(curr_centers_raw, axis=0) + np.ptp(next_centers_raw, axis=0)
            ...
            for j in range(comparison_data['cluster_count']):
                # Normalize each feature's difference by its range for fair comparison
                normalized_diff = (curr_centers_raw[j] - next_centers_raw[j]) / (feature_ranges / 2)
            ...
            stability_results.append({
                ...
                'Stability': "✅ Stable" if avg_distance < 1.0 else "⚠️ Changing"  # Adjusted threshold for raw space
            })
